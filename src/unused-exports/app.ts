...
import { TNotUsed, getNotUsed, sortNotUsedFn } from './notUsed'
...
const fixPath = (path: string, prefixLen: number): string => fixPathSeparator(path.substring(prefixLen))
...
export async function app(absPathToPrj: string, overviewContext: OverviewContext): Promise<TNotUsed[]> {
  ...
  const numNotUsedExports = finalList.reduce((acc, { notUsedExports }) => acc + (notUsedExports?.length ?? 0), 0)
  ts = log('🎯 Not used exports', numNotUsedExports, ts)
  finalList.forEach(({ filePath, notUsedExports }) => {
    notUsedExports?.forEach((name) => log('🎯 Not used export', `${filePath}: ${name}`))
  }, 0)

  const [unusedExportsAndCircularImportsList, numCircularImports] = await detectCircularImports(
    relations,
    finalList,
    ts,
  )
